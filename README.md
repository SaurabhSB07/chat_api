...
**Authentication & Token Helper Endpoints (provided by Simple JWT):**
...
```json
{
  ...
  "access": "eyJ0eXAiOiJKV1QiLCJh... (JWT token)",
  "refresh": "eyJ0eXAiOiJKV1QiLCJh... (JWT token)"
}
```
...
```json
{
  "response": "This is a dummy AI reply.",
  "tokens": 3900
}
```
...
Request:
```http
GET /api/tokenbalance/ (with Authorization: Bearer )
```
Response:
```json
{
...
- Integrating custom user model with Django authentication and Simple JWT library required careful migration and configuration changes.
- Handling token deduction to always ensure atomicity: tokens are only deducted when a chat message is properly stored.
- Ensuring robust error handling so invalid or expired tokens, duplicate usernames, and insufficient token cases are handled gracefully.

***

## Suggestions for Improvement

- Integrate a real AI chatbot model (such as OpenAI GPT or Rasa) instead of dummy replies for enhanced user interaction.
- Implement pagination and filtering for the chat history endpoint.
- Improve security and config management by using environment variables for secrets.
- Add more informative error messages and logging for better developer experience.
- Develop unit and integration tests to ensure API reliability and maintainability.
